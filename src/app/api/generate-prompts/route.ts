...
极其重要：每一条提示词都必须在开头明确包含「${styleConfig.visualStyle}」这个风格关键词。
...
- 每条提示词描述一个单一的静态画面
- 禁止动态叙述
- 提示词要丰富饱满，不要惜字如金。每条提示词至少 120 字，详细描述画面中的每一个视觉元素
- 提示词以中文输出

## 逻辑一致性（极其重要）

每条提示词只能有一个确定的视角/景别，不可自相矛盾：
- 如果是远景/全景，就描述广阔的场景，人物只是画面中的小元素
- 如果是中景，人物占画面主体，能看到上半身或全身
- 如果是近景/特写，只描述局部细节（脸、手、物品），不要同时描述全身
- 绝对禁止在同一条提示词中混合不同景别（如「一个人站在广场上，特写他的眼神」——这在一张静态图里无法实现）

## 每条提示词必须包含的要素

1. 风格关键词（开头）：${styleConfig.visualStyle}
2. 色彩基调：${styleConfig.colorTone === "正常" ? "自然色彩" : styleConfig.colorTone}
3. 明确的景别（远景/全景/中景/近景/特写/微距，只选一个）
4. 画面主体的具体描述（姿态、表情、动作、服装等细节）
5. 场景环境的丰富细节（地面材质、背景元素、天气、植物等）
6. 光影描述（光源方向、色温、阴影形态、高光位置）
7. 材质质感（物体表面的触感、纹理、温度感）
8. 氛围情绪（通过色调和元素传递的情感）
${infographicMode ? "9. 信息图元素（用户已开启智能信息图）\n" : ""}${skillText ? (infographicMode ? "10" : "9") + ". 风格参考中的关键风格特征\n" : ""}
## 提示词结构模板
${styleConfig.visualStyle}风格，${styleConfig.colorTone === "正常" ? "自然色彩" : styleConfig.colorTone}，[景别]，[画面主体及其姿态/表情/服装等细节]，[场景环境的丰富描写：地面/墙壁/天空/植物/道具等]，[光影：光源方向+色温+阴影]，[材质质感]，[氛围情绪]

以 JSON 数组返回，每个元素：
- id: 段落ID
- prompt: 提示词（120-200字，丰富饱满，逻辑自洽，必须包含风格和色调关键词）
